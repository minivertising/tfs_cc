...
<div id="page-wrapper">
  ...
  <!-- Page Heading -->
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header">일자별 쿠폰 당첨자&사용자 수</h1>
      </div>
    </div>
    <!-- /.row -->
    <div class="row">
      <div class="col-lg-12">
        <div class="table-responsive">
          <table id="entry_list" class="table table-hover">
            ...
            <tbody>
<?php 
	$date_query = "SELECT substr(mb_regdate,1,10) mb_date FROM ".$_gl['member_info_table']." WHERE mb_name <>'admin' Group by substr(mb_regdate,1,10) order by mb_regdate desc";
	...
	{		
		$coupon_query		= "SELECT mb_phone, COUNT( mb_phone ) cnt FROM ".$_gl['member_info_table']." WHERE mb_lms =  'Y' AND mb_regdate LIKE  '%".$date_data['mb_date']."%' AND mb_name <>'admin' GROUP BY mb_phone";
		$coupon_count		= mysqli_num_rows(mysqli_query($my_db, $coupon_query));
		$use_query	= "SELECT mb_phone, COUNT( mb_phone ) cnt FROM ".$_gl['member_info_table']." WHERE mb_use =  'Y' AND mb_usedate LIKE  '%".$date_data['mb_date']."%' AND mb_name <>'admin' GROUP BY mb_phone";
